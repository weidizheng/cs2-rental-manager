...
- 导入预览的“关联资产”列允许手工指定，选择结果写入 `item_id`，并记录 `match_method` 与置信度。选择“暂不关联”写入 `match_method=user_unlinked`，这是明确决定，不会在再次导入时被自动匹配覆盖；订单仍会保存，并可从资产页“未关联订单”查看。订单历史按 `item_id` 聚合；仅为兼容尚未迁移的旧记录才使用唯一磨损回退。
