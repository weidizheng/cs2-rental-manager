...
- 频控是进程级共享状态：大盘、求购、切页和手动同步创建的所有 `CSFloatClient` 共用最后请求时间、服务端自适应间隔和冷却截止时间。1.25 秒只是最低保护值；`RateLimit-Remaining/Reset` 会按“距重置时间 ÷ 剩余请求数”放慢全局频率，`Retry-After` 或 HTTP 429 会暂停整个调度队列。UI 会明确显示反馈来源和剩余秒数，不再每 12 秒启动 Worker 重复报告同一冷却。
...
- “立即同步”、大盘 `F5`、首次进入求购页和后台定时器全部调用 `_request_global_sync_now()` / `_run_rolling_market_refresh()`，不再直接启动各自页面的刷新。每个调度周期只处理到期求购或最久未更新的一件观察品；跨分类重复的“`market_hash_name` + 相位”只请求一次。
