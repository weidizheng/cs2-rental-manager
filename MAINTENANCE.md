...
| 系统与费率设置 | 保存 CSQAQ Token、CSFloat API Key/美元汇率、ECO Partner ID/RSA 私钥、资产页本地刷新间隔与费率 | 设置只写入私密数据目录 |
...
- 资产页“AI 批量导入”复制 `AI_ASSET_IMPORT_PROMPT`，接收 `items` JSON 后逐行通过 `CS2ItemSchema`、`InventoryItemDraft` 与平台/状态白名单校验；以“英文市场名 + 完整磨损值”跳过重复实物。写入后与手工资产共用成本、行情价差、租金价差和收益逻辑。
- 新购饰品选择 `CD冷却` 时必须输入 `0 < cooldown_hours <= 720`；仓储保存绝对 ISO 时间 `cooldown_until`，因此重启不会重置倒计时。`expire_hours` 仅保留输入/旧数据兼容。
...
- 频控采用三层保护：本地请求间隔至少 1.25 秒；读取 `RateLimit-Remaining`/`RateLimit-Reset`/`Retry-After` 并在 429 后停止；每轮最多请求 40 个未缓存饰品。超过预算的条目按最久未更新优先留到下一轮，避免大列表长时间连续请求。
...
- 手动“刷新行情”在后台 `QThread` 中运行 `MarketRefreshWorker`；全局自动同步则每个调度周期只刷新最久未更新的一件观察品。两者都覆盖所有分类，跨分类重复的“`market_hash_name` + 相位”只请求一次，并把行情字段复制回各分类。
- 资产、大盘、求购与设置页顶部的“数据同步”按钮都是同一个开关和同一进度状态。调度器严格串行：若 CSFloat 求购已到 10 分钟刷新周期则先读取求购，否则刷新一件行情；任一请求进行中都不会启动另一个。暂停后当前请求完成即停止。
...
- `W/S` 循环切换三个主工作区，大盘页用 `A/D` 切观察分类；输入框、多行 JSON 区和下拉框有焦点时不拦截 WASD。`Alt+1/2/3/4`、`Alt+←/→` 继续作为辅助快捷键；`Ctrl+N` 新增资产，`Ctrl+F` 聚焦搜索框，`F5` 手动刷新行情。表格保留原生方向键行为。
...
- `频控冷却中`：不要连续点击刷新。程序会遵守服务端响应头并在本次运行期间保留冷却状态。
