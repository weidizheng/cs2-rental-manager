...
- 资产页按平台筛选，统计买入总资产、当前每日净收益、累计净收益、在租件数和年化估算。资产平台不再提供“悠悠有品”；升级会把旧资产该字段留空，`CD冷却` 与 `在库`仍是资产状态而非平台。手动标记“已出租”但没有匹配订单时会计入在租件数并显示橙色“未导入订单”提示；没有真实订单到期时间时绝不伪造倒计时。
...
- 导入预览的“关联资产”列允许手工指定，选择结果写入 `item_id`，并记录 `match_method` 与置信度。无法唯一匹配时可选择“新建资产并关联”：软件只预填订单可靠给出的饰品名、磨损、相位、日租和当前出租状态，买入成本等无法从出租订单推断的数据必须在弹出的资产表单中确认。选择“暂不关联”写入 `match_method=user_unlinked`，这是明确决定，不会在再次导入时被自动匹配覆盖；订单仍会保存，并可从资产页“未关联订单”查看，随后可用“为选中订单新建资产”补建并关联。订单历史按 `item_id` 聚合；仅为兼容尚未迁移的旧记录才使用唯一磨损回退。
- 导入预览与订单历史的“日租（原价）”不扣费；资产页的“日租（净）”、本单净收入和累计净收益会对全部历史订单重新按费率计算。新订单距离前一单**租赁到期**少于 7 天（含直接交接/时间重叠）时使用转租费率，否则使用首次出租费率。
- 已由订单详情核对的默认口径：C5 首次/转租服务费均为 15%；ECO 首次/转租费率默认为 0。IGXE 页面没有可靠字段区分一键/手动定价，因此导入预览必须由用户逐笔确认：`one_click` 为 5%，`manual` 为 10%，该值持久化在订单上，优先于设置中的 IGXE 首次/转租费率；历史未确认订单仍回退配置值。连续出租的 `9 折` 会在服务费前先应用，结果按分向下截断。已验证一键样例：`2.32 × 8 × 0.9 × 0.95 = 15.8688 → ¥15.86`、`2.10 × 8 × 0.9 × 0.95 = 14.364 → ¥14.36`；未结算的手动样例 `2.62 × 8 × 0.9 × 0.90` 预计为 `¥16.97`，待结算金额出现后可继续校准。C5 的“转租奖励”不是通用转租费率，而是出租方支付给原承租方的独立成本。它以 C5 **原订单详情**为唯一依据：`最高奖励`只表示最多可能扣除多少，`待发放`仍未结算；两者都只展示、不进入净收益。若原订单为 `已转交`、仍未发放，并且同一饰品的下一笔 C5 订单已到期，订单历史的奖励列显示“待结算”，提醒复制**原订单**详情重新导入。只有详情同时为 `已转交` 和 `已发放` 时，才将页面实际金额（上限为原订单总金额的 5%）计入成本。`未转租`、`已取消`及 ECO/IGXE 一律为 0。这样不需要通过本地历史猜测具体奖励金额；同一订单重新导入会覆盖奖励状态并自动重算历史净收益。C5 纯文本和带 Markdown `**加粗**` 标记的复制格式均受支持。
- 双击资产行可以打开订单历史与单条详情。
...
`rental_orders` 使用 `(platform, order_no)` 唯一键，因此再次复制同一页面只更新订单而不会重复计算。资产和订单磨损统一保存为 **8 位小数、十进制四舍五入（5 入）**；不同平台的长短显示不再造成重复记录。首次升级会创建 `app.db.pre-float-normalization-时间.bak`，再统一旧记录。若“标准化名称 + 8 位磨损”相同，系统归并为同一资产，优先保留 C5GAME 的稳定 `asset_id`，并把所有订单历史转到主资产；合并来源平台会写入备注，而每笔订单原有的平台字段保持不变。关联按以下顺序进行：已确认的 `item_id`、稳定 `asset_id`、唯一的库存磨损，最后是已有资产历史中唯一的已关联磨损。历史关联**不按平台过滤**：一件饰品可在 A 平台买入、在 B 平台出租；本次导入的平台始终写入订单本身，不会覆盖资产的买入平台。若候选不唯一，用户点击“确认导入”后才会弹出资产选择框，也可选“暂不关联”继续；IGXE 页面缺少定价方式时同样在确认时弹窗询问。预览窗口已加大，定价方式与关联资产列固定为可见宽度，并移除无意义的左侧行号栏。与前一单**租赁到期**间隔少于 7 天的订单显示为“已转租”，间隔达到 7 天或没有前序订单则显示为“已出租”。任何解析失败或被用户在预览中取消的订单都不会写入数据库。
...
当前 SQLite `user_version` 为 **9**：v1 补齐旧版订单字段，v2 新增整数分金额并回填，v3 新增稳定订单关联与资产软删除，v4 将观察分类/观察品迁入主库，v5 为所有资产回填全局唯一 `asset_id`，v6 新增 `cooldown_until` 并为仍有有限 `expire_hours` 的旧 CD 资产回填绝对截止时间，v7 为订单新增用户确认的 `pricing_mode`，用于保存 IGXE 一键/手动定价，v8 将磨损统一为 8 位并合并精度导致的重复资产，v9 移除旧资产的“悠悠有品”平台分类。新版本发现数据库版本高于自身支持范围时会拒绝启动，避免旧程序误写新结构。
...
先确认资产记录的磨损值与订单页的磨损一致；旧资产磨损被截短时，订单关联沿用原有的末位半单位严格容差，不按中文名称猜测。匹配到多个资产时仍保持未关联，需在预览中手工指定，或选“暂不关联”直接保存并从“未关联订单”查看。C5 列表通常只给订单实际收入而不单列日租，因此程序按订单起止时间的整天数反算；ECO、IGXE 会优先保存订单页写明的原始日租。若平台改版导致“未解析到有效订单”，不要反复粘贴，先保存原始文本并据此更新 `rental_order_parsers.py`。
