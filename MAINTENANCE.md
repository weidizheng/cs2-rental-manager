...
`rental_orders` 使用 `(platform, order_no)` 唯一键，因此再次复制同一页面只更新订单而不会重复计算。关联按以下顺序进行：已确认的 `item_id`、稳定 `asset_id`、唯一的库存磨损，最后是已有资产历史中唯一的已关联磨损。磨损比较会保留双方共同有效的小数位，因此平台截短的显示磨损仍可匹配库存完整磨损。历史关联**不按平台过滤**：一件饰品可在 A 平台买入、在 B 平台出租；本次导入的平台始终写入订单本身，不会覆盖资产的买入平台。若候选不唯一，用户点击“确认导入”后才会弹出资产选择框，也可选“暂不关联”继续；IGXE 页面缺少定价方式时同样在确认时弹窗询问。预览窗口已加大，定价方式与关联资产列固定为可见宽度，并移除无意义的左侧行号栏。与前一单**租赁到期**间隔少于 7 天的订单显示为“已转租”，间隔达到 7 天或没有前序订单则显示为“已出租”。任何解析失败或被用户在预览中取消的订单都不会写入数据库。
