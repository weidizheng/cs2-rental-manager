...
- 不包含 AI OCR 自动录入功能。
- 程序不能在未授权情况下读取普通浏览器的登录 Cookie 或订单页 DOM；因此暂未能自动导入默认浏览器的订单。
...
2. 点击“打开 C5/ECO/IGXE 订单页”，在你已登录的默认浏览器中查看对应订单。
3. 当需要使用现有 C5 读取器时，在“C5 隔离登录（备用）”的独立浏览器中完成验证，再点“读取 C5 订单（隔离浏览器）”。
